...
pub use crate::pcap_common::{
    BpfProgram, PCapDumper, PCapHandle, PCapHandler, PCapInterface, PCapPacketHeader,
};
...
pub struct WPCapDll {
    ...
    pcap_setmintocopy: unsafe extern "C" fn(handle: *const PCapHandle, size: c_int) -> c_int,
    //devices
    pcap_findalldevs:
    ...
    pcap_sendqueue_queue: unsafe extern "C" fn(
        queue: *mut PCapSendQueue,
        pkt_head: *const PCapPacketHeader,
        pkt_data: *const u8,
    ) -> c_int,
    pcap_sendqueue_transmit: unsafe extern "C" fn(
        p: *const PCapHandle,
        queue: *mut PCapSendQueue,
        sync: c_int,
    ) -> c_uint,
