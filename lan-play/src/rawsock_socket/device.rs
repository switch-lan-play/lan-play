...
impl<S: Stream + Unpin, O: Sink<Packet> + Unpin> FutureDevice<S, O> {
    pub fn new(stream: S, output: O) -> NewFuture<S, O> {
        NewFuture(Some(Inner { stream, output }))
    }
    ...
    O: 'd,
{
    ...
    fn receive(&'d mut self) -> Option<(Self::RxToken, Self::TxToken)> {
        ...
        if let Poll::Ready(Some(p)) = Stream::poll_next(
            Pin::new(&mut s.stream),
            &mut Context::from_waker(&self.waker),
        ) {
            Some((FutureRxToken(p), FutureTxToken(self.inner.clone())))
        } else {
            None
        }
    }
...
pub trait AsyncIface<'b, 'c, 'e, S, O> {
    fn poll_async<'a>(
        &'a mut self,
        sockets: &'a mut SocketSet<'static, 'static, 'static>,
    ) -> PollAsync<'a, 'b, 'c, 'e, S, O>
    where
        S: Stream<Item = Packet> + Unpin + 'static,
        O: Sink<Packet> + Unpin + 'static;
}

impl<'b, 'c, 'e, S, O> AsyncIface<'b, 'c, 'e, S, O>
    for EthernetInterface<'b, 'c, 'e, FutureDevice<S, O>>
where
    S: Stream<Item = Packet> + Unpin + 'static,
    O: Sink<Packet> + Unpin + 'static,
{
